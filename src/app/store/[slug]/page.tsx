...
import { SartoriaHome } from '@/components/flagship-generic/milanese-tailoring-style/SartoriaHome'
...
import { RiotHome } from '@/components/flagship-generic/london-punk-style/RiotHome'
...
import { FlamencaHome } from '@/components/flagship-generic/andalusian-flamenco-style/FlamencaHome'
...
function StorePageRouter() {
  ...
  if (slug === 'reelrack') {
    ...
    return <>{banner}<ReelRackHome brand={b} products={REELRACK_PRODUCTS} /></>
  }
  if (slug === 'thegrid') {
    const b = previewName ? { ...THEGRID_BRAND, name: previewName } : THEGRID_BRAND
    return <>{banner}<TheGridHome brand={b} products={THEGRID_PRODUCTS} /></>
  }
  if (slug === 'tryiton') {
  ...
  if (slug === 'milanese-tailoring' && FLAGSHIP_REGISTRY[slug]) {
    const entry = FLAGSHIP_REGISTRY[slug]
    const b = previewName ? { ...entry.brand, name: previewName } : entry.brand
    return <>{banner}<SartoriaHome brand={b} products={entry.products} mechanicLabel={entry.mechanicLabel} mechanicIntro={entry.mechanicIntro} onOpenMechanic={() => window.dispatchEvent(new CustomEvent(OPEN_SARTORIA_MECHANIC_EVENT))} /></>
  }
  ...
  if (slug === 'london-punk' && FLAGSHIP_REGISTRY[slug]) {
    const entry = FLAGSHIP_REGISTRY[slug]
    const b = previewName ? { ...entry.brand, name: previewName } : entry.brand
    return <>{banner}<RiotHome brand={b} products={entry.products} mechanicLabel={entry.mechanicLabel} mechanicIntro={entry.mechanicIntro} onOpenMechanic={() => window.dispatchEvent(new CustomEvent(OPEN_RIOT_MECHANIC_EVENT))} /></>
  }
  ...
  if (slug === 'andalusian-flamenco' && FLAGSHIP_REGISTRY[slug]) {
    const entry = FLAGSHIP_REGISTRY[slug]
    const b = previewName ? { ...entry.brand, name: previewName } : entry.brand
    return <>{banner}<FlamencaHome brand={b} products={entry.products} mechanicLabel={entry.mechanicLabel} mechanicIntro={entry.mechanicIntro} onOpenMechanic={() => window.dispatchEvent(new CustomEvent(OPEN_FLAMENCA_MECHANIC_EVENT))} /></>
  }
...
function getDemoProducts(): Product[] {
  return [
    { id: 'demo-01', seller_id: 'demo', name: 'Satin Slip Maxi Dress', description: 'Fluid satin with adjustable straps. Runs true to size.', category: 'Dresses', price_inr: 2799, original_price_inr: 3499, garment_image_url: _U('photo-1496747611176-843222e1e57c'), garment_preprocessed_url: null, slug: 'satin-slip-maxi-dress', is_active: true, sizes: ['XS','S','M','L','XL'], colors: ['Champagne','Black','Sage'], tags: ['bestseller'], created_at: '' },
    { id: 'demo-02', seller_id: 'demo', name: 'Floral Wrap Midi Dress', description: 'Lightweight wrap silhouette, adjustable tie waist.', category: 'Dresses', price_inr: 1999, original_price_inr: null, garment_image_url: _U('photo-1572804013309-59a88b7e92f1'), garment_preprocessed_url: null, slug: 'floral-wrap-midi-dress', is_active: true, sizes: ['XS','S','M','L'], colors: ['Floral Print'], tags: [], created_at: '' },
    { id: 'demo-03', seller_id: 'demo', name: 'Off-Shoulder Mini Dress', description: 'Clean off-shoulder cut, structured bodice, stretch hem.', category: 'Dresses', price_inr: 1799, original_price_inr: null, garment_image_url: _U('photo-1496440737103-cd596325d314'), garment_preprocessed_url: null, slug: 'off-shoulder-mini-dress', is_active: true, sizes: ['XS','S','M','L','XL'], colors: ['White','Black'], tags: ['new'], created_at: '' },
    { id: 'demo-04', seller_id: 'demo', name: 'Oversized Linen Shirt', description: 'Boxy relaxed fit, 100% European linen, coconut buttons.', category: 'Tops', price_inr: 1499, original_price_inr: null, garment_image_url: _U('photo-1434389677669-e08b4cac3105'), garment_preprocessed_url: null, slug: 'oversized-linen-shirt', is_active: true, sizes: ['XS','S','M','L','XL','XXL'], colors: ['Ivory','Sand','Navy'], tags: [], created_at: '' },
    { id: 'demo-05', seller_id: 'demo', name: 'Cropped Ribbed Tank', description: 'Fitted crop in thick-rib jersey. Pairs with everything.', category: 'Tops', price_inr: 899, original_price_inr: null, garment_image_url: _U('photo-1509631179647-0177331693ae'), garment_preprocessed_url: null, slug: 'cropped-ribbed-tank', is_active: true, sizes: ['XS','S','M','L'], colors: ['Black','White','Cream','Mocha'], tags: ['bestseller'], created_at: '' },
    { id: 'demo-06', seller_id: 'demo', name: 'Satin Cowl-Neck Cami', description: 'Draped cowl neck, adjustable straps, relaxed fit.', category: 'Tops', price_inr: 1299, original_price_inr: 1699, garment_image_url: _U('photo-1469334031218-e382a71b716b'), garment_preprocessed_url: null, slug: 'satin-cowl-neck-cami', is_active: true, sizes: ['XS','S','M','L','XL'], colors: ['Champagne','Dusty Rose','Slate'], tags: ['sale'], created_at: '' },
    { id: 'demo-07', seller_id: 'demo', name: 'High-Rise Straight Jeans', description: 'Classic high-rise with a straight leg. 98% cotton, slight stretch.', category: 'Denim', price_inr: 2499, original_price_inr: null, garment_image_url: _U('photo-1542272604-787c3835535d'), garment_preprocessed_url: null, slug: 'high-rise-straight-jeans', is_active: true, sizes: ['24','25','26','27','28','29','30','31','32'], colors: ['Mid Wash','Dark Wash','Light Wash'], tags: ['bestseller'], created_at: '' },
    { id: 'demo-08', seller_id: 'demo', name: 'Wide-Leg Barrel Jeans', description: 'Relaxed barrel silhouette, low to mid rise.', category: 'Denim', price_inr: 2799, original_price_inr: null, garment_image_url: _U('photo-1475178626620-a4d074967452'), garment_preprocessed_url: null, slug: 'wide-leg-barrel-jeans', is_active: true, sizes: ['24','25','26','27','28','29','30'], colors: ['Ecru','Mid Wash'], tags: ['new'], created_at: '' },
    { id: 'demo-09', seller_id: 'demo', name: 'Classic Denim Jacket', description: 'Structured denim jacket with chest pockets and button closure.', category: 'Denim', price_inr: 2999, original_price_inr: 3799, garment_image_url: _U('photo-1591047139829-d91aecb6caea'), garment_preprocessed_url: null, slug: 'classic-denim-jacket', is_active: true, sizes: ['XS','S','M','L','XL'], colors: ['Light Wash','Dark Wash'], tags: ['sale'], created_at: '' },
    { id: 'demo-10', seller_id: 'demo', name: 'Camel Trench Coat', description: 'Belted trench in a water-repellent shell. Timeless silhouette.', category: 'Outerwear', price_inr: 5499, original_price_inr: null, garment_image_url: _U('photo-1539533018447-63fcce2678e3'), garment_preprocessed_url: null, slug: 'camel-trench-coat', is_active: true, sizes: ['XS','S','M','L','XL'], colors: ['Camel','Khaki'], tags: ['new'], created_at: '' },
    { id: 'demo-11', seller_id: 'demo', name: 'Oversized Wool Blazer', description: 'Boyfriend-fit blazer in a herringbone wool blend.', category: 'Outerwear', price_inr: 4299, original_price_inr: 5499, garment_image_url: _U('photo-1490481651871-ab68de25d43d'), garment_preprocessed_url: null, slug: 'oversized-wool-blazer', is_active: true, sizes: ['XS','S','M','L','XL'], colors: ['Black','Charcoal','Cream'], tags: ['sale'], created_at: '' },
    { id: 'demo-12', seller_id: 'demo', name: 'Quilted Puffer Vest', description: 'Lightweight quilted fill, cropped length, snap buttons.', category: 'Outerwear', price_inr: 2299, original_price_inr: null, garment_image_url: _U('photo-1548624313-0396c75e4b1a'), garment_preprocessed_url: null, slug: 'quilted-puffer-vest', is_active: true, sizes: ['XS','S','M','L','XL'], colors: ['Black','Cream','Olive'], tags: [], created_at: '' },
    { id: 'demo-13', seller_id: 'demo', name: 'Mini Crossbody Bag', description: 'Structured mini bag in pebbled faux leather. Adjustable strap.', category: 'Accessories', price_inr: 1999, original_price_inr: null, garment_image_url: _U('photo-1548036328-c9fa89d128fa'), garment_preprocessed_url: null, slug: 'mini-crossbody-bag', is_active: true, sizes: [], colors: ['Black','Tan','Burgundy'], tags: ['bestseller'], created_at: '' },
    { id: 'demo-14', seller_id: 'demo', name: 'Gold Chain Hoops', description: 'Lightweight chunky chain hoops, 18k gold-plated brass.', category: 'Accessories', price_inr: 899, original_price_inr: null, garment_image_url: _U('photo-1606760227091-3dd870d97f1d'), garment_preprocessed_url: null, slug: 'gold-chain-hoops', is_active: true, sizes: [], colors: ['Gold'], tags: ['new'], created_at: '' },
    { id: 'demo-15', seller_id: 'demo', name: 'Ribbed Knit Beanie', description: 'Chunky ribbed knit in a one-size-fits-all style.', category: 'Accessories', price_inr: 699, original_price_inr: null, garment_image_url: _U('photo-1516762689617-e1cffcef479d'), garment_preprocessed_url: null, slug: 'ribbed-knit-beanie', is_active: true, sizes: [], colors: ['Oatmeal','Black','Camel','Sage'], tags: [], created_at: '' },
  ]
}
