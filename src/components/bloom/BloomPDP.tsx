...
export function BloomPDP({ brand, product, related = [] }: { brand: ThemeBrand; product: ThemeProduct; related?: ThemeProduct[] }) {
  ...
  useEffect(() => {
    recordProductView('bloom_viewed_v1', product.id)
  }, [product.id])
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-6 md:px-10 pt-8 pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16">
        <div className="relative aspect-[4/5] md:aspect-auto md:h-[calc(100vh-8rem)] md:sticky md:top-24 overflow-hidden rounded-xl" style={{ background: 'var(--bl-card)' }}>
        ...
        <div className="pt-2">
          ...
          {product.colors.length > 0 && (
            <div className="mb-6">
              <p className="text-xs tracking-wide uppercase mb-2.5" style={{ color: 'var(--bl-ink-dim)' }}>Colour — {color}</p>
              <div className="flex gap-2">
                {product.colors.map(c => (
                  <button
                    ...
                    style={color === c ? { background: 'var(--bl-ink)', color: 'var(--bl-bg)', borderColor: 'var(--bl-ink)' } : { borderColor: 'var(--bl-line)' }}
                  >
                    {c}
                  </button>
                ))}
          ...
          {product.sizes.length > 0 && (
            <div className="mb-8">
              <p className="text-xs tracking-wide uppercase mb-2.5" style={{ color: 'var(--bl-ink-dim)' }}>Size</p>
              <div className="flex gap-2 flex-wrap">
                {product.sizes.map(s => (
                  <button
                    ...
                    style={size === s ? { background: 'var(--bl-ink)', color: 'var(--bl-bg)', borderColor: 'var(--bl-ink)' } : { borderColor: 'var(--bl-line)' }}
                  >
                    {s}
                  </button>
                ))}
              </div>
              {product.fit && <p className="text-xs mt-2.5" style={{ color: 'var(--bl-ink-dim)' }}>{product.fit}</p>}
            </div>
          )}
          ...
          <p className="text-[11px] text-center mb-8" style={{ color: 'var(--bl-ink-dim)' }}>Free shipping &amp; returns · Ships in 2–4 business days</p>

          {(product.detail || product.fabric || product.fit) && (
            <div className="border-t" style={{ borderColor: 'var(--bl-line)' }}>
              {(['detail', 'fit'] as const).map(panel => (
                <div key={panel} className="border-b" style={{ borderColor: 'var(--bl-line)' }}>
                  <button
                  ...
                  {openPanel === panel && (
                    <div className="pb-4 text-sm leading-relaxed" style={{ color: 'var(--bl-ink-muted)' }}>
                      {panel === 'detail' ? (
                        <>
      ...
      {related.length > 0 && (
        <section className="mt-24">
          <Reveal>
            <h2 className="bloom-display italic text-3xl md:text-4xl mb-8">You may also consider</h2>
          </Reveal>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5 md:gap-8">
            {related.map((p, i) => (
              <Reveal key={p.id} delay={i * 0.06}>
                <BloomProductCard product={p} slug={slug} />
              </Reveal>
            ))}
          </div>
        </section>
