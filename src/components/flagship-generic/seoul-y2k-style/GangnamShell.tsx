...
import { GangnamHeader } from './GangnamHeader'
...
export const OPEN_GANGNAM_MECHANIC_EVENT = 'flagship-gangnam:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_GANGNAM_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_GANGNAM_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <GangnamHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function GangnamShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
