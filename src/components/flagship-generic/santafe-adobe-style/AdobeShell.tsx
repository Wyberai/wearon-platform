...
export const OPEN_ADOBE_MECHANIC_EVENT = 'flagship-adobe:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_ADOBE_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_ADOBE_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <AdobeHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function AdobeShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
