...
import { SoukHeader } from './SoukHeader'
...
export const OPEN_SOUK_MECHANIC_EVENT = 'flagship-souk:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_SOUK_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_SOUK_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <SoukHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function SoukShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
