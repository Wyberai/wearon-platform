...
export const OPEN_OCEAN_MECHANIC_EVENT = 'flagship-ocean:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_OCEAN_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_OCEAN_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <OceanHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function OceanShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
