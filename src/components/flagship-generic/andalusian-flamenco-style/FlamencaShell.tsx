...
import { FlamencaHeader } from './FlamencaHeader'
...
export const OPEN_FLAMENCA_MECHANIC_EVENT = 'flagship-flamenca:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_FLAMENCA_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_FLAMENCA_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <FlamencaHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function FlamencaShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
