...
export const OPEN_RESORT_MECHANIC_EVENT = 'flagship-resort:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_RESORT_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_RESORT_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <ResortHeader brand={entry.brand} categories={entry.brand.categories} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function ResortShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
