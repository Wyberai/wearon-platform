...
import { FjordHeader } from './FjordHeader'
...
export const OPEN_FJORD_MECHANIC_EVENT = 'flagship-fjord:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_FJORD_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_FJORD_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <FjordHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function FjordShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
