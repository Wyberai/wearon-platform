...
import { UbudHeader } from './UbudHeader'
...
export const OPEN_UBUD_MECHANIC_EVENT = 'flagship-ubud:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_UBUD_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_UBUD_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <UbudHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function UbudShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
