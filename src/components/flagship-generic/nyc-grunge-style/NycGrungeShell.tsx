...
import { NycGrungeHeader } from './NycGrungeHeader'
...
export const OPEN_NYCGRUNGE_MECHANIC_EVENT = 'flagship-nycgrunge:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_NYCGRUNGE_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_NYCGRUNGE_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <NycGrungeHeader brand={entry.brand} categories={entry.brand.categories} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function NycGrungeShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
