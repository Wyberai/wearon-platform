...
import { RiotHeader } from './RiotHeader'
...
export const OPEN_RIOT_MECHANIC_EVENT = 'flagship-riot:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_RIOT_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_RIOT_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <RiotHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function RiotShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
