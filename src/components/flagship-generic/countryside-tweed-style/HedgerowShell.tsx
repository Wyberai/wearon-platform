...
import { HedgerowHeader } from './HedgerowHeader'
...
export const OPEN_HEDGEROW_MECHANIC_EVENT = 'flagship-hedgerow:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_HEDGEROW_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_HEDGEROW_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <HedgerowHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function HedgerowShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
