...
import { KintsugiHeader } from './KintsugiHeader'
...
export const OPEN_KINTSUGI_MECHANIC_EVENT = 'flagship-kintsugi:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_KINTSUGI_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_KINTSUGI_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <KintsugiHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function KintsugiShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
