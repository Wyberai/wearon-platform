...
import { StijlHeader } from './StijlHeader'
...
export const OPEN_STIJL_MECHANIC_EVENT = 'flagship-stijl:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_STIJL_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_STIJL_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <StijlHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function StijlShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
