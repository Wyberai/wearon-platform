...
import { ShibuyaHeader } from './ShibuyaHeader'
...
export const OPEN_SHIBUYA_MECHANIC_EVENT = 'flagship-shibuya:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_SHIBUYA_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_SHIBUYA_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <ShibuyaHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function ShibuyaShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
