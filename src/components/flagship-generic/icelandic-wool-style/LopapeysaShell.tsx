...
import { LopapeysaHeader } from './LopapeysaHeader'
...
export const OPEN_LOPAPEYSA_MECHANIC_EVENT = 'flagship-lopapeysa:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_LOPAPEYSA_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_LOPAPEYSA_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <LopapeysaHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function LopapeysaShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
