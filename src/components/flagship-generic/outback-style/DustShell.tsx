...
import { DustHeader } from './DustHeader'
...
export const OPEN_DUST_MECHANIC_EVENT = 'flagship-dust:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_DUST_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_DUST_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <DustHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function DustShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
