...
export const OPEN_CAMPUS_MECHANIC_EVENT = 'flagship-campus:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_CAMPUS_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_CAMPUS_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <CampusHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function CampusShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
