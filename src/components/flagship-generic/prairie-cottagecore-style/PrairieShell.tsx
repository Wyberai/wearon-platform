...
import { PrairieHeader } from './PrairieHeader'
...
export const OPEN_PRAIRIE_MECHANIC_EVENT = 'flagship-prairie:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_PRAIRIE_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_PRAIRIE_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <PrairieHeader brand={entry.brand} categories={entry.brand.categories} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function PrairieShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
