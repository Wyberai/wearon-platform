...
import { WerkstattHeader } from './WerkstattHeader'
...
export const OPEN_WERKSTATT_MECHANIC_EVENT = 'flagship-werkstatt:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_WERKSTATT_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_WERKSTATT_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <WerkstattHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function WerkstattShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
