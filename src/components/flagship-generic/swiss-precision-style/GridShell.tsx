...
export const OPEN_GRID_MECHANIC_EVENT = 'flagship-grid:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_GRID_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_GRID_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <GridHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function GridShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
