...
import { GlenmoreHeader } from './GlenmoreHeader'
...
export const OPEN_GLENMORE_MECHANIC_EVENT = 'flagship-glenmore:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_GLENMORE_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_GLENMORE_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <GlenmoreHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function GlenmoreShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
