...
import { RiveHeader } from './RiveHeader'
...
export const OPEN_RIVE_MECHANIC_EVENT = 'flagship-rive:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_RIVE_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_RIVE_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <RiveHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function RiveShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
