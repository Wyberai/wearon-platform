...
import { MagnoliaHeader } from './MagnoliaHeader'
...
export const OPEN_MAGNOLIA_MECHANIC_EVENT = 'flagship-magnolia:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_MAGNOLIA_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_MAGNOLIA_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <MagnoliaHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function MagnoliaShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
