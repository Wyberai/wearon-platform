...
export const OPEN_NOIR_MECHANIC_EVENT = 'flagship-noir:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  // Only the home route (/store/[slug], no further segments) has the tall
  // hero this overlay treatment is built for — every other route (shop,
  // PDP, checkout) gets the header in its normal solid state.
  const isHome = /^\/store\/[^/]+\/?$/.test(pathname ?? '')

  useEffect(() => {
    function handler() { setMechanicOpen(true) }
    window.addEventListener(OPEN_NOIR_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_NOIR_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <NoirHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} overlay={isHome} />
...
export function NoirShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
