...
// A faithful clone of injiri.com's real chrome: a hamburger icon
// (left), a wordmark set in wide letter-spacing (center), and search/
// bag icons (right) — quiet chrome that lets the craft photography
// carry the page.
export function RajwadaHeader({ brand, mechanicLabel, onOpenMechanic }: { brand: ThemeBrand; mechanicLabel: string; onOpenMechanic: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b" style={{ background: 'var(--fg-bg)', borderColor: 'var(--fg-line)' }}>
      <div className="px-4 md:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <Link href={`/store/${slug}`} className="text-lg tracking-[0.5em] uppercase" style={{ color: 'var(--fg-ink)', fontWeight: 500 }}>
          {brand.name.split(' ')[0]}
        </Link>
