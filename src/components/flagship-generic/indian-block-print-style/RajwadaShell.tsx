...
import { RajwadaHeader } from './RajwadaHeader'
...
export const OPEN_RAJWADA_MECHANIC_EVENT = 'flagship-rajwada:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_RAJWADA_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_RAJWADA_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <RajwadaHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function RajwadaShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
