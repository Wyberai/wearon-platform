...
import { HyggeHeader } from './HyggeHeader'
...
export const OPEN_HYGGE_MECHANIC_EVENT = 'flagship-hygge:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_HYGGE_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_HYGGE_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <HyggeHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function HyggeShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
