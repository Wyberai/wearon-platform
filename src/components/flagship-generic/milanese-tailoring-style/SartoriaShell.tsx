...
import { SartoriaHeader } from './SartoriaHeader'
...
export const OPEN_SARTORIA_MECHANIC_EVENT = 'flagship-sartoria:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_SARTORIA_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_SARTORIA_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <SartoriaHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function SartoriaShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
