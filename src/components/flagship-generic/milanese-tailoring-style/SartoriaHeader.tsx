...
const ANNOUNCEMENTS = ['Complimentary Alterations In Store', 'Book a Private Fitting', 'Free Shipping on Orders Over $500']

// A faithful clone of loropiana.com's real chrome: a rotating cream
// announcement strip with prev/next arrows sits above the header proper,
// which is hamburger + search (left), a crest-style wordmark (center),
// and stylist + bag icons (right) — the same "service before commerce"
// framing as the real site.
export function SartoriaHeader({ brand, mechanicLabel, onOpenMechanic }: { brand: ThemeBrand; mechanicLabel: string; onOpenMechanic: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-50" style={{ background: 'var(--fg-bg)' }}>
      <div className="h-9 flex items-center justify-center gap-4 border-b text-[11px] tracking-[0.05em]" style={{ borderColor: 'var(--fg-line)', color: 'var(--fg-ink-dim)' }}>
        <button aria-label="Previous" onClick={() => setI((i - 1 + ANNOUNCEMENTS.length) % ANNOUNCEMENTS.length)} className="hover:opacity-60">&lsaquo;</button>
        <span>{ANNOUNCEMENTS[i]}</span>
        <button aria-label="Next" onClick={() => setI((i + 1) % ANNOUNCEMENTS.length)} className="hover:opacity-60">&rsaquo;</button>
      </div>
      <div className="px-6 md:px-10 h-16 flex items-center justify-between gap-6 border-b" style={{ borderColor: 'var(--fg-accent)' }}>
        ...
        <Link href={`/store/${slug}`} className="text-2xl italic" style={{ color: 'var(--fg-accent)', fontWeight: 500 }}>
          {brand.name}
        </Link>
