...
import { VeniceHeader } from './VeniceHeader'
...
export const OPEN_VENICE_MECHANIC_EVENT = 'flagship-venice:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_VENICE_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_VENICE_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <VeniceHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function VeniceShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
