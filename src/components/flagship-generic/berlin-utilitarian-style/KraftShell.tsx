...
export const OPEN_KRAFT_MECHANIC_EVENT = 'flagship-kraft:open-mechanic'
...
function ShellInner({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_KRAFT_MECHANIC_EVENT, handler)
    return () => window.removeEventListener(OPEN_KRAFT_MECHANIC_EVENT, handler)
  }, [])
  ...
  return (
    <div style={flagshipCssVars(entry.palette, entry.font)} className="flex flex-col">
      ...
      <KraftHeader brand={entry.brand} mechanicLabel={entry.mechanicLabel} onOpenMechanic={() => setMechanicOpen(true)} />
...
export function KraftShell({ children, entry }: { children: ReactNode; entry: FlagshipEntry }) {
